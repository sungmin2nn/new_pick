...
### 가설
당일 강세 업종의 시가총액 상위 종목은 추가 상승 가능성이 높다

### 기존 전략과의 차이
Delta Theme와 다름: Delta는 naver 테마(당일) + KRX 업종(과거) 혼합 사용, 매핑 정합성 약함. 본 전략은 KRX 업종 지수만 단일 소스로 사용하여 섹터 → 시총 → 거래량 3단계 깔때기를 적용. 데이터 소스가 일관되어 백테스트 재현성이 높고, 분봉 불필요.

### 기대 수익 원천
섹터 자금 유입의 일시적 추세, 대장주 효과
...
### 경고 / 주의
- ⚠️ Sharpe 38.0 > 15.0 → 짧은 기간 과대평가 의심
...
code zip1/strategy-lab/strategies/sector_rotation.py
```
...
### Step 2: 파일 복사
```bash
cp zip1/strategy-lab/strategies/sector_rotation.py \
   zip1/news-trading-bot/paper_trading/strategies/sector_rotation.py
```
...
"team_f": {
    "team_id": "team_f",
    "team_name": "Foxtrot SectorRotationStrategy",
    "strategy_id": "sector_rotation",
    "emoji": "⚡",
    "description": "섹터 로테이션 (KOSPI 업종)",
},
```
...
- **Sharpe 과대평가 가능**: Sharpe 38.0는 1주 기간 특성
...
1. **[verified]** KRX 업종 지수 일별 시세 (idx/kospi_dd_trd)
   (type: official_doc)
   > KRX OpenAPI 공식 엔드포인트.
2. **[high]** news-trading-bot-handoff.md Section 6.2
   (type: handoff_guide)
   > 신규 전략 후보로 명시 (Sector Rotation).
...
**승급 평가 파일**: `data/promotions/sector_rotation_20260411.json`
