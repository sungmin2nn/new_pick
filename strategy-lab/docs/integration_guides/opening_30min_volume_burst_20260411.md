...
### 가설
개장 직후 거래대금 폭발 + 양봉 종목은 일중 추세 지속

### 기존 전략과의 차이
Echo Frontier와 다름: Echo는 가격 갭(2~5%)이 시그널, 본 전략은 거래대금 surge가 시그널. Volatility BO와 다름: VB는 가격 변동폭, 본 전략은 거래량/거래대금 변동. Alpha Momentum과 다름: Alpha는 5일 평균 + 거래량 3배 (스무딩), 본 전략은 단일일 거래대금 폭발 강도 자체. 근거: 한국 시장 골든타임 관행 + MIM 학술.

### 기대 수익 원천
한국 시장 고유의 개장 골든타임 거래대금 집중 효과
...
### 경고 / 주의
- ⚠️ Sharpe 56.5 > 15.0 → 짧은 기간 과대평가 의심
...
code zip1/strategy-lab/strategies/opening_30min_volume_burst.py
```
...
### Step 2: 파일 복사
```bash
cp zip1/strategy-lab/strategies/opening_30min_volume_burst.py \
   zip1/news-trading-bot/paper_trading/strategies/opening_30min_volume_burst.py
```
...
```python
from .opening_30min_volume_burst import Opening30minVolumeBurstStrategy
__all__ = [..., 'Opening30minVolumeBurstStrategy']
```
...
"team_f": {
    "team_id": "team_f",
    "team_name": "Foxtrot Opening30minVolumeBurstStrategy",
    "strategy_id": "opening_30min_volume_burst",
    "emoji": "⚡",
    "description": "개장 30분 거래대금 폭발 (한국 골든타임)",
},
```
...
- **Sharpe 과대평가 가능**: Sharpe 56.5는 1주 기간 특성
...
1. **[medium]** 주식투자/단타매매 기법
   https://namu.wiki/w/주식투자/단타매매%20기법
   (type: blog_ko)
   > 나무위키. 다수 trader 검증된 단타 관행 요약.
2. **[verified]** Market Intraday Momentum (KOSPI evidence) — Park & Yang
   https://www.mdpi.com/1911-8074/15/11/523
   (type: paper)
   > 첫 30분 시그널 R² 3.3% 검증 (변동성 ↑ 시).
...
**승급 평가 파일**: `data/promotions/opening_30min_volume_burst_20260411.json`
